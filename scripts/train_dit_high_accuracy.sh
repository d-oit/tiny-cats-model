...
if [ "$1" == "--local" ]; then
    ...
    echo "Mode: PRODUCTION (100k steps with early stopping)"
    echo ""
    echo "🚀 Optimized Training Configuration:"
    echo "  - Batch size: 512 (better gradient estimates)"
    echo "  - Warmup: 2k steps (faster convergence)"
    echo "  - Early stopping: Auto-stop when loss plateaus"
    echo "  - Max steps: 100k (often stops at 50-80k)"
    echo ""
    echo "Estimated cost: €8-15 (vs €25 for fixed 400k)"
    echo ""
    echo "Use GitHub Actions for reliable training:"
    echo "  gh workflow run train.yml"
    echo ""
    echo "The workflow now defaults to optimized settings:"
    echo "  - steps: 100000 (with early stopping)"
    echo "  - batch_size: 512"
    echo "  - lr: 5e-5"
